...
extension WebPostRequestBrick: CBInstructionProtocol {

    static let alertSemaphore = DispatchSemaphore(value: 1)

    @nonobjc func instruction() -> CBInstruction {
        if WebPostRequestBrick.isWebRequestBrickEnabled() {
            ...
            return CBInstruction.waitExecClosure { context, scheduler in
                ...
                if let requestBody = self.requestBody {
                    bodyString = context.formulaInterpreter.interpretString(requestBody, for: self.script.object)
                } else {
                    bodyString = ""
                }

                let downloader = self.downloaderFactory.create(url: requestString, trustedDomainManager: trustedDomains, httpMethod: "POST", httpBody: bodyString)
