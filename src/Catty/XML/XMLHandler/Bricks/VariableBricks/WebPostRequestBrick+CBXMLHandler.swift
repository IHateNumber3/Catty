...
extension WebPostRequestBrick: CBXMLNodeProtocol {
    static func parse(from xmlElement: GDataXMLElement, with context: CBXMLParserContext) -> Self {
        CBXMLParserHelper.validate(xmlElement, forFormulaListWithTotalNumberOfFormulas: 2)

        let brick = self.init()
        let formula = CBXMLParserHelper.formula(in: xmlElement, forCategoryName: "WEB_REQUEST", with: context)
        brick.request = formula
        let bodyFormula = CBXMLParserHelper.formula(in: xmlElement, forCategoryName: "WEB_REQUEST_BODY", with: context)
        brick.requestBody = bodyFormula
    ...
    func xmlElement(with context: CBXMLSerializerContext) -> GDataXMLElement? {
        let brick = super.xmlElement(for: "WebPostRequestBrick", with: context)
        ...
        let bodyFormula = self.requestBody?.xmlElement(with: context)
        bodyFormula?.addAttribute(GDataXMLElement(name: "category", stringValue: "WEB_REQUEST_BODY", context: nil))
        formulaList?.addChild(bodyFormula, context: context)
