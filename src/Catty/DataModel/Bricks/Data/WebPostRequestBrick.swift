...
@objc(WebPostRequestBrick)
@objcMembers class WebPostRequestBrick: Brick, BrickProtocol, BrickFormulaProtocol, BrickVariableProtocol {

    var request: Formula?
    var requestBody: Formula?
    ...
    convenience init(request: Formula, requestBody: Formula, userVariable: UserVariable, script: Script) {
        self.init()
    ...
    override func brickCell() -> BrickCellProtocol.Type! {
        WebPostRequestBrickCell.self as BrickCellProtocol.Type
    }
    ...
    func formula(forLineNumber lineNumber: Int, andParameterNumber paramNumber: Int) -> Formula! {
        paramNumber == 0 ? self.request : self.requestBody
    }
    ...
    func getFormulas() -> [Formula]? {
        ...
        if let request = request {
            formulas.append(request)
        }
        if let requestBody = requestBody {
            formulas.append(requestBody)
        }
        return formulas.isEmpty ? nil : formulas
    }

    override func setDefaultValuesFor(_ spriteObject: SpriteObject!) {
        self.request = Formula(string: "https://catrob.at/joke")
        self.requestBody = Formula(string: "{}")
    }
